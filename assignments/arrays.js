...
console.log(`Make and model of last car in inventory is ${inventory[49].car_make} ${inventory[49].car_model}.`);
...
const carModelsSorted = carModels.sort();
console.log(carModelsSorted);

// ==== Challenge 4 ====
// The accounting team needs all the years from every car on the lot. Create a new array from the dealer data containing only the car years and log the result in the console.
let carYears = [];
 /*Regular*/ for (let i = 0; i < inventory.length; i++) {
    carYears.push(inventory[i].car_year);
}
//Stretch:
carYears.push(inventory.map(item => item.car_year));
...
// Stretch way:
const year = inventory.filter(i => i.car_year < 2000);
...
const add2NumCallback = (x, y) => {
    return x + y;
};

const checkConsistentOutputHigherOrder = (myfunc, value1, value2) => {
    let call1 = myfunc(value1, value2);
    let call2 = myfunc(value1, value2);
    if (call1 === call2) {
      return call1;
    } else {
  ...
  console.log(checkConsistentOutputHigherOrder(add2NumCallback, 1, 2));

const logTheResultHigherOrder = (myfunc, value1, value2) => {
    console.log(myfunc(value1, value2));
};

logTheResultHigherOrder(add2NumCallback, 5, 6);
